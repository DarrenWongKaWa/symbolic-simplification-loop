...
Before running any construction loop, read `docs/safety.md`.

The forbidden-path denylist in `docs/safety.md` is authoritative for this
probe. Construction loops must not modify scientific / runtime / checkpoint /
signoff paths unless a bounded task explicitly allows it and the human
approves.

No auto-commit, auto-freeze, or auto-signoff is allowed. No push without
explicit human approval. No `git add .`.
